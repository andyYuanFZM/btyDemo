package com.chain33.cn.deployByUser;
...
//    String withholdPrivateKey = "替换成自己的私钥，用下面createAccount方法生成,注意私钥千万不能泄漏";
    ...
    public void testERC1155() throws Exception {
        ...
        // 部署合约, 参数： 平行链合约名， 合约地址，合约部署人地址，合约部署人私钥，代扣私钥
        String hash = deployContractByNoBalance(execer, paracontractAddress, useraAddress, useraPrivateKey, withholdPrivateKey);
        
        // 计算上一步部署到链上的合约地址
        String contractAddress = client.convertExectoAddr(useraAddress + hash.substring(2));
    ...
    private String deployContractByNoBalance(String execer, String paracontractAddress, String userAddress, String userPrivateKey, String withHoldPrivateKey) throws Exception {
    	...
    	// 再调用代扣交易方法，用代扣私钥对交易组做签名
    	String txhash = createNobalance(txEncode, paracontractAddress, userPrivateKey, withHoldPrivateKey);
